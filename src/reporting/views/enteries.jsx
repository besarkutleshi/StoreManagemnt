...
const EnteriesReports = () => {
    ...
    useEffect(async () => {
        ...
        setLoading(true);
        let result = await repCtrl.getProfits("monthProfits/2");
        if (result) {
            setMonthsProfits(result);
        }
        result = await repCtrl.getProfits("yearProfits/2");
        if(result){
            setYearsProfits(result)
        }
        result = await repCtrl.getProfits("itemProfits/2");
        if(result){
            setItemsProfits(result);
        }
        result = await repCtrl.getProfits("collaborationProfits/2");
        if(result){
            setCollaborationProfits(result);
        }
        result = await repCtrl.getProfits("invoicesProfit");
        if(result){
            setInvoices(result);
        }
        result = await repCtrl.getProfits("weekProfits");
        if(result){
            console.log(result);
...
export default EnteriesReports